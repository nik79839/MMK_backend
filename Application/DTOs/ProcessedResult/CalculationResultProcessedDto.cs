    ...
    public class CalculationResultProcessedDto
    {
        ...
        public void Processing(List<VoltageResult> voltageResults)
        {
            ...
            foreach (var nodeNumber in nodeNumbers)
            {
                ...
                List<HistogramDataDto> histogramData = new();
                for (int i = 0; i < intervalCount; i++)
                {
                    ...
                    histogramData.Add(new HistogramDataDto() { Interval = Math.Round(first, 2).ToString() + " - " + Math.Round(sec, 2).ToString(), Height = height });
                }
                VoltageResultProcessed.Add(new VoltageResultProcessedDto() { Maximum = maximum, Minimum = minimum, Mean = mean, HistogramData = histogramData, NodeNumber = nodeNumber });
            }
        }

        public static StatisticBaseDto GetStatistic(List<double> values)
        {
            StatisticBaseDto statisticBase = new();
            int intervalCount = Convert.ToInt32(Math.Log10(values.Count) + Math.Sqrt(values.Count));
            ...
            for (int i = 0; i < intervalCount; i++)
            {
                ...
                height = Convert.ToDouble(count) / Convert.ToDouble(values.Count) / step;
                statisticBase.HistogramData.Add(new HistogramDataDto() { Interval = Math.Round(first, 2).ToString() + " - " + Math.Round(sec, 2).ToString(), Height = height });
            }
